...
macOS Apple Silicon GitHub 发布包由 `apps/desktop` 的 `dist:mac:internal` 负责构建；当前 `v0.1.4` 是使用 ad-hoc 签名、未经过 Apple 公证的可追溯发布包。面向普通用户的 Apple 正式发行仍需要 Developer ID 和 Apple 公证凭据，不能把当前包描述为已公证版本。
