...
(async () => {
    ...
    Object.defineProperty(window, 'myScholarDesktop', {
      ...
      value: Object.freeze({
        ...
        getStartupContext: async () => ({
          ok: true,
          app: { name: '谷子学术', version: '0.1.4', platform: 'darwin', arch: 'arm64', channel: 'internal' },
          storage: {
            state: 'conflict',
            adopted: false,
        ...
        checkForUpdates: async () => {
          ...
          if (window.__updateCheckCount === 1) return { ok: true, status: 'current', currentVersion: '0.1.4', version: '0.1.4', checkedAt: new Date().toISOString() };
          return {
            ok: true,
            status: 'available',
  ...
  await page.waitForFunction(() => document.querySelector('#app-current-version')?.textContent.trim() === 'v0.1.4');
  ...
  await page.locator('#check-updates').click();
  await page.waitForFunction(() => document.querySelector('#update-status-title')?.textContent.trim() === '发现新版本 v0.1.4');
